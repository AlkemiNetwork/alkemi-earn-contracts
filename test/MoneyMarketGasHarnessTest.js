...
contract("MoneyMarketGasHarness", function ([root, ...accounts]) {
	...
	describe("harnessSetAccountSupplyBalance", async () => {
		it("stores a balance for about the cost of two words stored @gas", async () => {
			const moneyMarketHarness = await MoneyMarketGasHarness.new().send({
				from: root,
			});
			...
			const first = await moneyMarketHarness.methods
			...
			const second = await moneyMarketHarness.methods
				.harnessSetAccountSupplyBalance(
					account,
					asset,
					principal + 1,
