...
contract("MoneyMarket", function ([root, ...accounts]) {
	describe("admin / _setPendingAdmin", async () => {
		it("admin is initially set to root and pendingAdmin is 0", async () => {
			...
			assert.equal(
				addressZero,
				await moneyMarket.methods.pendingAdmin().call(),
	...
	describe("admin / _acceptAdmin", async () => {
		...
		it("succeeds if called by pendingAdmin", async () => {
			...
			assert.matchesAddress(
				accounts[1],
				await moneyMarket.methods.admin().call()
			);
	...
	describe("oracle / _setOracle", async () => {
		it("is initially unset", async () => {
			...
			assert.matchesAddress(
				addressZero,
				await moneyMarket.methods.oracle().call()
			);
		});
		...
		// 	await assert.revert(
		// 		await moneyMarket.methods._setOracle(accounts[1]).send({ from: root })
		// 	);
		// });
	});
	...
	// 				const moneyMarket = await MoneyMarket.new().send({ from: root });
	// 				await readAndExecContract(moneyMarket, "initializer", [], {
	// 					from: root,
	// 				});
	// 				const priceOracle = await PriceOracle.new().send({ from: root });
	...
	// 				await moneyMarket.methods.addKYCAdmin(root).send({ from: root });
	...
	// 							market.methods
	// 								.approve(moneyMarket._address, 100)
	// 								.send({ from: customer }),
	// 						]);
	// 					})
	// 				);
	...
	// 				// Prime the pump
	// 				const result1 = await moneyMarket.methods[method](
	// 					markets[0]._address,
	// 					1
	// 				).send({ from: customer });
	...
	// 				// And again
	// 				const result2 = await moneyMarket.methods[method](
	// 					markets[0]._address,
	// 					2
	// 				).send({ from: customer });
