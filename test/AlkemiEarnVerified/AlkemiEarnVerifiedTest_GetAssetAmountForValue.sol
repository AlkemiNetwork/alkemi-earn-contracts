...
contract AlkemiEarnVerifiedTest_GetAssetAmountForValue is
    AlkemiEarnVerifiedWithPriceTest
{
    ...
    function testGetAssetAmountForValue_ZeroCases() public {
        ...
        (err, amount) = getAssetAmountForValue(
            asset,
            getExpFromRational(100, 1)
        ); // 100 eth worth

        Assert.equal(
            uint256(Error.DIVISION_BY_ZERO),
            uint256(err),
            "should have returned Error.DIVISION_BY_ZERO"
        );
    ...
    function testHandlesUnsetPriceOracle() public {
        ...
        (Error err, uint256 result) = getAssetAmountForValue(
            asset,
            getExpFromRational(1070, 1)
        );
        assertError(
