...
/*
 * @dev This tests the Alkemi Earn Verified with tests for supportMarket part 3.
 */
contract AlkemiEarnVerifiedTest_SupportMarket3 is
    AlkemiEarnVerifiedWithPriceTest
{
    function testSupportMarket_FailsIfPriceNotAlreadySet() public {
        ...
        assertError(
            ...
            "expected Error.ASSET_NOT_PRICED"
        );

        Assert.equal(
            markets[asset].isSupported,
            false,
            "market should not be supported"
        );
        Assert.equal(
            markets[asset].interestRateModel,
            address(0),
            "market has no interest rate model set"
        );
    }
