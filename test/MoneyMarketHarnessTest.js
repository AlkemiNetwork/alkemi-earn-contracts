...
contract("MoneyMarketHarness", function([root, ...accounts]) {
	describe("_supportMarket", async () => {
		...
		it("succeeds and sets market", async () => {
			...
			assert.equal(
				Number(await moneyMarket.methods.getCollateralMarketsLength().call()),
				0
			);
			...
			assert.equal(
				await moneyMarket.methods.collateralMarkets(0).call(),
				asset,
				"should have collateral market"
			);
		});
		...
		it("succeeds works a second time", async () => {
			...
			const [_errorCode, _tx, _error] = await readAndExecContract(
				moneyMarket,
				"_supportMarket",
				[asset, interestRateModel._address],
				{ from: root }
			);
			const [errorCode, tx, _error2] = await readAndExecContract(
				moneyMarket,
				"_supportMarket",
				[asset, interestRateModel._address],
				{ from: root }
			...
			assert.withinPercentage(
				1e18,
				Number(await moneyMarket.methods.harnessGetSupplyIndex(asset).call()),
				1e-10,
				"should have starting supply index"
			);
			assert.withinPercentage(
				1e18,
				Number(await moneyMarket.methods.harnessGetBorrowIndex(asset).call()),
				1e-10,
				"should have starting borrow index"
			);
		});
		...
		it("adds two assets", async () => {
			...
			const drgn = await EIP20.new(100, "drgn", 18, "drgn").send({
				from: root,
			});
			const price = getExpMantissa(0.5).toString(10);

			await moneyMarket.methods
				.harnessSetAssetPriceMantissa(omg._address, price)
				.send({ from: root });
			await moneyMarket.methods
				.harnessSetAssetPriceMantissa(drgn._address, price)
				.send({ from: root });

			const [errorCode0, tx0, _error0] = await readAndExecContract(
				moneyMarket,
				"_supportMarket",
				[omg._address, interestRateModel._address],
				{ from: root }
			);
			const [errorCode1, tx1, _error1] = await readAndExecContract(
				moneyMarket,
				"_supportMarket",
				[drgn._address, interestRateModel._address],
				{ from: root }
			);
			...
			assert.equal(
				await moneyMarket.methods.collateralMarkets(0).call(),
				checksum(omg._address),
				"should have omg market"
			);
			assert.equal(
				await moneyMarket.methods.collateralMarkets(1).call(),
				checksum(drgn._address),
				"should have drgn market"
			);

			assert.withinPercentage(
				1e18,
				Number(
					await moneyMarket.methods.harnessGetSupplyIndex(omg._address).call()
				),
				1e-10,
				"omg should have starting supply index"
			);
			assert.withinPercentage(
				1e18,
				Number(
					await moneyMarket.methods.harnessGetSupplyIndex(drgn._address).call()
				),
				1e-10,
				"drgn should have starting supply index"
			);
		});
	});
	...
	describe("_setMarketInterestRateModel", async () => {
		it("fails if not called by admin", async () => {
			...
			assert.matchesAddress(
				await moneyMarket.methods.harnessGetInterestRateModel(asset).call(),
	...
	describe("_withdrawEquity", async () => {
		it("fails if not called by admin", async () => {
			...
			const result = await moneyMarket.methods
		...
		it("fails if amount requested exceeds equity", async () => {
			...
			// Give protocol cash
			await OMG.methods
				.harnessSetBalance(moneyMarket._address, 10000)
				.send({ from: root });
	...
	describe("_setRiskParameters", async () => {
		it("emits log on failure due to non-admin caller", async () => {
			...
			const minCollatRatio = getExpMantissa(1.25);
			const maxLiqRatio = getExpMantissa(0.1);

			const result = await moneyMarket.methods
				._setRiskParameters(
					newRatio.toString(10),
					newDiscount.toString(10),
					minCollatRatio.toString(10),
					maxLiqRatio.toString(10)
				)
				.send({ from: accounts[1], gas: 1000000 });
		...
		it("emits log on failure due to invalid liquidation discount", async () => {
			...
			const newRatio = getExpMantissa(3.5);
			const newDiscount = getExpMantissa(0.2);
			const minCollatRatio = getExpMantissa(1.25);
			const maxLiqRatio = getExpMantissa(0.1);
		...
		// 	const newRatio = 13 * 10 ** 17;
		// 	const newDiscount = 3 * 10 ** 17;
		// 	const minCollatRatio = getExpMantissa(1.25);
		// 	const maxLiqRatio = getExpMantissa(0.1);
		...
		it("emits log on successful change", async () => {
			...
			assert.hasLog(tx, "NewRiskParameters", {
				oldCollateralRatioMantissa: getExpMantissa(1.25).toString(),
				newCollateralRatioMantissa: newRatio.toString(),
				oldLiquidationDiscountMantissa: getExpMantissa(0.1).toString(),
	...
	describe("_setOriginationFee", async () => {
		it("emits log on failure due to non-admin caller", async () => {
			...
			const result = await moneyMarket.methods
				._setOriginationFee(newRatio.toString(10))
		...
		it("emits log on successful change", async () => {
			...
			assert.hasLog(tx, "NewOriginationFee", {
				oldOriginationFeeMantissa: getExpMantissa(0.001).toString(),
	...
	describe("harnessCalculateInterestIndex", async () => {
		it("calculates correct value for exact value", async () => {
			const moneyMarketHarness = await MoneyMarketHarness.new().send({
				from: root,
			});
			await readAndExecContract(moneyMarketHarness, "initializer", [], {
				from: root,
			});
			...
			assert.equal(
				6e18,
				Number(
					await moneyMarketHarness.methods
	...
	describe("supply", async () => {
		it("returns error and logs info if contract is paused", async () => {
			...
			await readAndExecContract(moneyMarketHarness, "addKYCAdmin", [root], {
				from: root,
			});
			...
			await OMG.methods
				.approve(moneyMarketHarness._address, 95)
				.send({ from: customer });

			await moneyMarketHarness.methods._setPaused(true).send({ from: root });

			const result = await moneyMarketHarness.methods
		...
		it("returns error if customer total new balance calculation fails", async () => {
			...
			// Customer now approves our Money Market to spend its value
			await OMG.methods
				.approve(moneyMarketHarness._address, bigNums.maxUint.toString(10))
				.send({ from: customer });
		...
		it("emits supply event on success", async () => {
			...
			assert.hasLog(result, "SupplyReceived", {
			...
			const customerCompoundBalance = await moneyMarketHarness.methods
				.supplyBalances(customer, OMG._address)
				.call();
	...
	describe("withdraw", async () => {
		it("returns error and logs info if contract is paused", async () => {
			...
			// Give protocol cash
			await OMG.methods
				.harnessSetBalance(moneyMarketHarness._address, 500)
				.send({ from: root });

			// Give customer some liquidity and an OMG supply balance
			await moneyMarketHarness.methods
				.harnessSetLiquiditySurplus(customer, 50)
				.send({ from: root });
			await moneyMarketHarness.methods
				.harnessSetAccountSupplyBalance(customer, OMG._address, 100, 1)
				.send({ from: root });
		...
		it("returns error and logs info if protocol has insufficient cash for withdrawal", async () => {
			...
			await OMG.methods
				.harnessSetBalance(moneyMarketHarness._address, 1)
				.send({ from: root });
		...
		it("returns error and logs info if account liquidity calculation fails", async () => {
			...
			// Give protocol cash
			await OMG.methods
				.harnessSetBalance(moneyMarketHarness._address, 100)
				.send({ from: root });
		...
		it("returns error and logs info if account is underwater", async () => {
			...
			await OMG.methods
				...
				.send({ from: root });
			await moneyMarketHarness.methods
				.harnessSetMarketDetails(OMG._address, 1000, 0, 1, 0, 0, 1)
				.send({ from: root });

			// Give customer a balance of OMG but also a liquidity shortfall
			await moneyMarketHarness.methods
				.harnessSetAccountSupplyBalance(customer, OMG._address, 20, 1)
				.send({ from: root });
			await moneyMarketHarness.methods
				.harnessSetLiquidityShortfall(customer, (10 ** 18).toString(10))
				.send({ from: root });
		...
		it("returns error and logs info if calculation of eth value of withdrawal fails", async () => {
			...
			await moneyMarketHarness.methods
				.harnessSetMaxAssetPrice(OMG._address)
				.send({ from: root });
		...
		it("returns an error and logs info if withdrawal would put account underwater", async () => {
			...
			const liquidity = 10;
			await moneyMarketHarness.methods
				.harnessSetLiquiditySurplus(customer, liquidity)
				.send({ from: root });
		...
		it("returns error and logs info if new supply interest index calculation fails", async () => {
			...
			await moneyMarketHarness.methods
				.harnessSetLiquiditySurplus(customer, 100)
				.send({ from: root });
		...
		it("returns error and logs info if accumulated balance calculation fails", async () => {
			...
			// To reach that we also have to set the previous principal to a non-zero value otherwise we will short circuit.
			await moneyMarketHarness.methods
				.harnessSetAccountSupplyBalance(customer, OMG._address, 100, 0)
				.send({ from: root });
		...
		it("returns error and logs info if customer total new balance calculation fails", async () => {
			...
			await moneyMarketHarness.methods
				.harnessSetAccountSupplyBalance(customer, OMG._address, 10, 1)
				.send({ from: root });
      ...
      */
			...
			await moneyMarketHarness.methods
				.harnessSetAccountSupplyBalance(customer, OMG._address, 10000, 1)
				.send({ from: root });
			...
			await moneyMarketHarness.methods
				.harnessSetAssetPrice(OMG._address, 1, 10)
				.send({ from: root });
		...
		it("returns error and logs info if new supply rate calculation fails", async () => {
			...
			// Configure market state for OMG: a supply of 1000 and supply and borrow indexes of 1.
			await moneyMarketHarness.methods
				.harnessSetMarketDetails(OMG._address, 1000, 1, 1, 0, 0, 1)
				.send({ from: root });
		...
		it("emits SupplyWithdrawn event on success", async () => {
			...
			assert.hasLog(tx, "SupplyWithdrawn", {
				account: checksum(customer),
				asset: OMG._address,
				amount: "1",
				startingBalance: "100",
		...
		it("special withdraw max value allows partial token withdrawal when customer has limited liquidity", async () => {
			...
			// Give customer some liquidity and an OMG supply balance
			await moneyMarketHarness.methods
				.harnessSetLiquiditySurplus(customer, 35)
				.send({ from: root });
			await moneyMarketHarness.methods
				.harnessSetAccountSupplyBalance(customer, OMG._address, 100, 1)
				.send({ from: root });
		...
		it("special withdraw max value returns an error if withdraw capacity calculation fails", async () => {
			...
			await moneyMarketHarness.methods
				.harnessSetAssetPrice(OMG._address, 0, 1)
				.send({ from: root });
		...
		it("allows withdraw of zero for customer with 0 liquidity and 0 shortfall", async () => {
			...
			await moneyMarketHarness.methods
				.harnessSetLiquiditySurplus(customer, 0)
				.send({ from: root });
			await moneyMarketHarness.methods
				.harnessSetLiquidityShortfall(customer, 0)
				.send({ from: root });
			await moneyMarketHarness.methods
				.harnessSetAccountSupplyBalance(customer, OMG._address, 100, 1)
				.send({ from: root });
		...
		it("disallows withdraw of zero for customer with a positive shortfall", async () => {
			...
			await moneyMarketHarness.methods
				.harnessSetLiquidityShortfall(customer, 35)
				.send({ from: root });
			await moneyMarketHarness.methods
				.harnessSetAccountSupplyBalance(customer, OMG._address, 100, 1)
				.send({ from: root });
	...
	describe("getSupplyBalance", async () => {
		it("returns balance in happy path when there is no interest to accumulate", async () => {
			...
			await moneyMarketHarness.methods
				.harnessSetAccountSupplyBalanceAndMarketDetails(
					customer,
					asset,
					principal,
			...
			const customerBalance = await moneyMarketHarness.methods
				.getSupplyBalance(customer, asset)
				.call();
		...
		it("returns balance in happy path when there IS interest to accumulate", async () => {
			...
			const customerBalance = await moneyMarketHarness.methods
				.getSupplyBalance(customer, asset)
				.call();
			assert.equal(customerBalance, 110);
		});
		...
		it("returns 0 on an unsupported market", async () => {
			...
			const customerBalance = await moneyMarketHarness.methods
				.getSupplyBalance(customer, OMG._address)
				.call();
		...
		it("returns 0 balance for supported market where customer has no balance", async () => {
			...
			const customerBalance = await moneyMarketHarness.methods
				.getSupplyBalance(customer2, asset)
				.call();
			assert.equal(customerBalance, 0);
		});
	});
	...
	describe("getBorrowBalance", async () => {
		it("returns balance in happy path when there is no interest to accumulate", async () => {
			...
			await moneyMarketHarness.methods
				.harnessSetAccountBorrowBalanceAndMarketDetails(
					customer,
					asset,
					principal,
					customerInterestIndex,
			...
			const customerBalance = await moneyMarketHarness.methods
				.getBorrowBalance(customer, asset)
				.call();
		...
		it("returns 0 on an unsupported market", async () => {
			...
			const customerBalance = await moneyMarketHarness.methods
				.getBorrowBalance(customer, OMG._address)
				.call();
			assert.equal(customerBalance, 0);
		});
		...
		it("returns 0 balance for supported market where customer has no balance", async () => {
			...
			const customerBalance = await moneyMarketHarness.methods
				.getBorrowBalance(customer2, asset)
				.call();
			assert.equal(customerBalance, 0);
		});
	});
	...
	describe("harnessCalculateAccountLiquidity", async () => {
		...
		[
			...
			{
				name:
					"Supply 1 OMG Wei, Borrow 1 ETH Wei. Prices (ETH: 1.0, OMG: 0.5) & collatRatio = 1",
				tokens: { ETH: 1.0, OMG: 0.5 },
				balances: { supply: { OMG: 1 }, borrow: { ETH: 1 } },
				collateralRatio: 1.0,
			...
			},
			{
				...
				name:
					"Supply 75 OMG, Borrow 1.5 ETH. Prices (ETH: 1.0, OMG: 0.018206) & collatRatio = 1.5",
				tokens: { ETH: 1.0, OMG: 0.018206 },
			...
			},
			{
				...
				name:
					"Supply 8 ETH, Borrow 100 OMG. Prices (ETH: 1.0, OMG: 0.018206) & collatRatio = 3",
				tokens: { ETH: 1.0, OMG: 0.018206 },
			...
			it(args["name"], async () => {
							...
							return Promise.all(
								Object.entries(balanceValue).map(([assetKey, assetAmount]) => {
									...
									if (balanceKey === "supply") {
										...
										return moneyMarketHarness.methods
											.harnessSetAccountSupplyBalance(
												account,
												assets[assetKey],
				...
				const liquidity = Number(
					await moneyMarketHarness.methods.getAccountLiquidity(account).call()
				);
				const expLiq =
		...
		it("returns account liquidity when there IS Supply interest to accumulate", async () => {
			...
			await readAndExecContract(moneyMarketHarness, "addCustomerKYC", [root], {
				from: root,
			});
			await moneyMarketHarness.methods
				.harnessSetCollateralRatio(2, 1)
	...
	describe("repayBorrow", async () => {
		...
		it("returns error if new supply interest index calculation fails", async () => {
			...
			// Give user some balance
			await moneyMarketHarness.methods
				.harnessSetAccountBorrowBalance(customer, OMG._address, 100, 2)
				.send({ from: root });
		...
		it("handles special pay max value when borrow balance exceeds customer token balance", async () => {
			...
			// Customer now approves our Money Market to spend its value
			await OMG.methods
				.approve(moneyMarketHarness._address, 100)
				.send({ from: customer });
	...
	async function setupValidLiquidation(nonStandard = false) {
		...
		const deliverBorrowAssetResult = await moneyMarketHarness.methods
			.supply(
				borrowed._address,
				liquidationSetup.initialTokenBalanceOtherSupplier
			)
		...
		await collateral.methods
			...
			.send({ from: borrower });
		const collateralSupplyResult = await moneyMarketHarness.methods
		...
		const borrowerCollateralBalance = await moneyMarketHarness.methods
		...
		// Create the borrow
		const borrowResult = await moneyMarketHarness.methods
		...
		const borrowBalance = await moneyMarketHarness.methods
	...
	async function validateFailedLiquidation(
		...
		const liquidatorCollateralBalance = await moneyMarketHarness.methods
			.supplyBalances(liquidator, collateral._address)
			.call();
		assert.equal(0, liquidatorCollateralBalance.principal);

		const borrowerBorrowBalance = await moneyMarketHarness.methods
			.borrowBalances(borrower, borrowed._address)
			.call();
	...
	describe("liquidateBorrow", async () => {
		it("returns error and logs info if contract is paused", async () => {
			...
			const liquidateResult = await moneyMarketHarness.methods
		...
		it("handles liquidator failure to approve borrowed asset for transfer in to protocol", async () => {
			...
			await borrowed.methods
				.approve(moneyMarketHarness._address, 0)
				.send({ from: liquidator });
	...
	describe("borrow", async () => {
		...
		it("returns error if borrow fee calculation fails", async () => {
			...
			await moneyMarketHarness.methods
				._setOriginationFee(getExpMantissa(10).toString(10))
				.send({ from: root });
		...
		it("returns an error if borrow amount value calcuation fails", async () => {
			...
			await moneyMarketHarness.methods
				._setOriginationFee(getExpMantissa(0))
				.send({ from: root });
		...
		it("returns error if token transfer fails", async () => {
			...
			await OMG.methods
				.transfer(moneyMarketHarness._address, 100)
				.send({ gas: tokenTransferGas, from: root });
		...
		it("succeeds, fails and succeeds", async () => {
			...
			const result0 = await moneyMarketHarness.methods
				.borrow(OMG._address, 5)
				.send({ from: customer });
			...
			const customerCompoundBalance0 = await moneyMarketHarness.methods
				.borrowBalances(customer, OMG._address)
				.call();
			...
			const result1 = await moneyMarketHarness.methods
			...
			const customerCompoundBalance1 = await moneyMarketHarness.methods
				.borrowBalances(customer, OMG._address)
				.call();
			...
			const result2 = await moneyMarketHarness.methods
			...
			const customerCompoundBalance2 = await moneyMarketHarness.methods
				.borrowBalances(customer, OMG._address)
				.call();
