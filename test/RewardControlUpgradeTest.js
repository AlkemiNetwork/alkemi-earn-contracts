...
contract('RewardControl upgrade test', function ([root, ...accounts]) {
    ...
    describe("#upgrade using openzeppelin", async () => {
        it("upgrade without the need to transfer ALK balance", async () => {
            ...
            const rewardControlV1 = await deployProxy(RewardControl, [root, accounts[2], ALK._address], {initializer: 'initializer'});
            console.log(`rewardControl.address: ${rewardControlV1.address}`);

            await ALK.methods.transfer(rewardControlV1.address, BigInt("70000000000000000000000000")).send({from: root});
            assert.equal((await ALK.methods.balanceOf(rewardControlV1.address).call()).toString(), "70000000000000000000000000");

            await rewardControlV1.harnessTransferAlk(accounts[1], BigInt("10000000000000000000000000"));
            ...
            // when
            const rewardControlV2 = await upgradeProxy(rewardControlV1.address, RewardControlV2);
            ...
            await rewardControlV2.harnessTransferAlk(accounts[2], BigInt("20000000000000000000000000"));
