...
contract("SafeTokenHarness", function ([root, ...accounts]) {
	describe("#checkInboundTransfer", async () => {
		it("abides by sane gas limits @gas", async () => {
			const safeTokenHarness = await SafeTokenHarness.new().send({
				from: root,
			});
			...
			// Account 1 now approves our Alkemi Earn Verified (SafeTokenHarness) to spend its value
			await OMG.methods
				.approve(safeTokenHarness._address, 100)
			...
			const first = await safeTokenHarness.methods
				.checkInboundTransfer(OMG._address, accounts[1], 80)
			...
			assert.withinGas(
				first,
				5 * gas.storage_read + 2 * estimatedExternalCallCost + unknownCost,
				5000,
				"should cost about 5 reads"
			);
		});
	});
	...
	describe("#doInboundTransfer", async () => {
		it("abides by sane gas limits @gas", async () => {
			...
			const first = await safeTokenHarness.methods
				.doInboundTransfer(OMG._address, accounts[1], 80)
				.send({ from: accounts[2] });

			// Perform a second transfer
			const second = await safeTokenHarness.methods
				.doInboundTransfer(OMG._address, accounts[1], 10)
				.send({ from: accounts[2] });

			// Account 1 now approves our Alkemi Earn Verified (SafeTokenHarness) to spend all its value (which does not need to be updated since it stays -1)
			await OMG.methods
				.approve(safeTokenHarness._address, -1)
				.send({ from: accounts[1] });

			// Perform a third transfer
			const third = await safeTokenHarness.methods
				.doInboundTransfer(OMG._address, accounts[1], 1)
				.send({ from: accounts[2] });

			// Verify everything worked as expected
			assert.equal(Number(await OMG.methods.balanceOf(root).call()), 0);
			assert.equal(
				Number(await OMG.methods.balanceOf(safeTokenHarness._address).call()),
				91
			);
			...
			assert.withinGas(
				...
				gas.storage_new +
					...
					unknownCost,
				5000,
				"should cost about one new storage and two updates"
			);
			...
			assert.withinGas(
				...
				3 * gas.storage_update +
					...
					unknownCost,
				5000,
				"should cost about three storage updates (allowance and two balances)"
			);
			...
			assert.withinGas(
				...
				2 * gas.storage_update +
					...
					unknownCost,
				5000,
				"should cost about two storage updates (two balances but NOT allowance)"
			);
		});
	});
	...
	describe("#doOutboundTransfer", async () => {
		it("abides by sane gas limits @gas", async () => {
			...
			await OMG.methods
				.transfer(safeTokenHarness._address, 100)
				.send({ gas: 1000000, from: root });

			// Since its now approved, let's pick some random recipient (account 1) transfer out via our exposed "safeTransfer" function
			const first = await safeTokenHarness.methods
				.doOutboundTransfer(OMG._address, accounts[2], 80)
				.send({ from: accounts[1] });

			// Perform a second transfer
			const second = await safeTokenHarness.methods
				.doOutboundTransfer(OMG._address, accounts[2], 10)
				.send({ from: accounts[1] });
			...
			assert.withinGas(
				...
				gas.storage_new +
					...
					2 * gas.storage_read +
					estimatedLogCost +
					1 * estimatedExternalCallCost,
				5000,
				"should cost about one new storage and one updates"
			);
			...
			assert.withinGas(
				...
				2 * gas.storage_update +
					...
					2 * estimatedExternalCallCost,
				5000,
				"should cost about two storage updates"
			);
		});
	});
	...
	describe("for a non-compliant (64-byte returning) token", async () => {
		it("reverts on #doTransferIn", async () => {
			...
			const OMG = await EIP20NonCompliant.new(
				100,
				"test omg nc",
				18,
				"omg"
			...
			await assert.revert(
				safeTokenHarness.methods
					.doInboundTransfer(OMG._address, accounts[0], 100)
