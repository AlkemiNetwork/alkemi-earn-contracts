...
assert.success = function (result) {
	if (result.events["Failure"]) {
		...
		assert.fail(
			0,
			1,
			`Expected success: got failure ${failure} (Error: ${error}, FailureInfo: ${failureInfo})`
		);
	}
...
assert.hasFailure = function (
	...
	if (!log) {
		...
		assert.fail(
			0,
			1,
			`Expected log with event Failure, found logs with events: [${events}]`
		);
	}
...
assert.revert = async function (trx, reason = "revert") {
	...
	try {
		...
		assert.equal(
			`Returned error: VM Exception while processing transaction: ${reason}`,
			err.message
		);
	...
	assert.fail(
		0,
		1,
		`expected revert, instead got result: ${JSON.stringify(result)}`
	);
};
...
assert.hasErrorCode = function (actualErrorCode, expectedErrorCode) {
	assert.equal(
		actualErrorCode,
		expectedErrorCode,
		`expected Error.${ErrorEnumInv[expectedErrorCode]} (error code ${expectedErrorCode}), instead got Error.${ErrorEnumInv[actualErrorCode]} (error code ${actualErrorCode})`
	);
};
