...
contract('RewardControl internal functions test', function ([root, ...accounts]) {
    ...
    describe("#updateAlkSupplyIndex", async () => {
        it("update supply state on the first time successfully", async () => {
            ...
            await rewardControl.methods.harnessSetAlkSpeed(accounts[1], "4161910200000000000", true).send({
                gas: 1000000,
                from: root
            });
            ...
            await rewardControl.methods.harnessUpdateAlkSupplyIndex(accounts[1], true).send({
                gas: 1000000, from: root
            });
            let response = await rewardControl.methods.alkSupplyState(true,accounts[1]).call();
        ...
        it("update supply state on the second time on the next block successfully", async () => {
            ...
            await rewardControl.methods.harnessSetAlkSupplyState(accounts[1], "41619102000000000000000000000000000000000000000000000", 1, true).send({
                gas: 1000000,
                from: root
            });
            await rewardControl.methods.harnessSetMarketTotalSupply(accounts[1], "100").send({
            ...
            await rewardControl.methods.harnessUpdateAlkSupplyIndex(accounts[1], true).send({gas: 1000000, from: root});
            let response = await rewardControl.methods.alkSupplyState(true, accounts[1]).call();
            let index = response[0];
            let block = response[1];
    ...
    describe("#updateAlkBorrowIndex", async () => {
        it("update borrow state on the first time successfully", async () => {
            ...
            await rewardControl.methods.harnessUpdateAlkBorrowIndex(accounts[1],true).send({gas: 1000000, from: root});
            let response = await rewardControl.methods.alkBorrowState(true,accounts[1]).call();
            let index = response[0];
            let block = response[1];
            assert.equal(index.toString(), "41619102000000000000000000000000000000000000000000000");
            assert.equal(block.toString(), "1");
        ...
        it("update borrow state on the second time on the next block successfully", async () => {
            ...
            await rewardControl.methods.harnessSetAlkBorrowState(accounts[1], "41619102000000000000000000000000000000000000000000000", 1, true).send({
                gas: 1000000,
                from: root
            });
            await rewardControl.methods.harnessSetMarketTotalBorrows(accounts[1], "100").send({
   ...
   describe("#distributeSupplierAlk", async () => {
        it("update accrued alk for a supplier on the first time successfully", async () => {
            ...
            await rewardControl.methods.harnessDistributeSupplierAlk(accounts[1], accounts[2], true).send({
                gas: 1000000,
                from: root
            });
            let alkAccrued = await rewardControl.methods.alkAccrued(accounts[2]).call();
            assert.equal(alkAccrued.toString(), "0");
            let alkSupplierIndex = await rewardControl.methods.alkSupplierIndex(true,accounts[1], accounts[2]).call();
        ...
        it("update accrued alk for a supplier on the second time successfully", async () => {
            const rewardControl = await RewardControl.new().send({from: root});
            await rewardControl.methods.harnessSetAlkSupplyState(accounts[1], "83238204000000000000000000000000000000000000000000000", "1", true).send({
                gas: 1000000,
                from: root
            });
            await rewardControl.methods.harnessSetAlkSupplierIndex(accounts[1], accounts[2], "41619102000000000000000000000000000000000000000000000", true).send({
                gas: 1000000,
                from: root
            });
            await rewardControl.methods.harnessSetSupplyBalance(accounts[1], accounts[2], "25").send({
    ...
    describe("#distributeBorrowerAlk", async () => {
        it("update accrued alk for a borrower on the first time successfully", async () => {
            ...
            await rewardControl.methods.harnessDistributeBorrowerAlk(accounts[1], accounts[2], true).send({
                gas: 1000000,
                from: root
            });
            let alkAccrued = await rewardControl.methods.alkAccrued(accounts[2]).call();
            assert.equal(alkAccrued.toString(), "0");
            let alkBorrowerIndex = await rewardControl.methods.alkBorrowerIndex(true,accounts[1], accounts[2]).call();
        ...
        it("update accrued alk for a borrower on the second time successfully", async () => {
            const rewardControl = await RewardControl.new().send({from: root});
            await rewardControl.methods.harnessSetAlkBorrowState(accounts[1], "83238204000000000000000000000000000000000000000000000", "1", true).send({
                gas: 1000000,
                from: root
            });
            await rewardControl.methods.harnessSetAlkBorrowerIndex(accounts[1], accounts[2], "41619102000000000000000000000000000000000000000000000", true).send({
                gas: 1000000,
                from: root
            });
            await rewardControl.methods.harnessSetBorrowBalance(accounts[1], accounts[2], "25").send({
    ...
    describe("#transferAlk", async () => {
        it("transfer ALK when RewardControl has no balance", async () => {
            ...
            await rewardControl.methods.initializer(root, accounts[2], accounts[3], ALK._address).send({gas: 1000000, from: root});

            await rewardControl.methods.harnessTransferAlk(accounts[1], "1040477550000000000", accounts[5], true).send({
                gas: 1000000,
                from: root
            });
    ...
    describe("#transferOwnership", async () => {
        it("transfer OwnerShip successfully", async () => {
            const rewardControl = await RewardControl.new().send({from: root});
            await rewardControl.methods.initializer(root, accounts[2], accounts[3], accounts[4]).send({gas: 1000000, from: root});
    ...
    describe("#{add|remove}Market", async () => {
        ...
        it("add/remove market failed without permission", async () => {
            const rewardControl = await RewardControl.new().send({from: root});
            await rewardControl.methods.initializer(root, accounts[2], accounts[3], accounts[4]).send({gas: 1000000, from: root});
            assert.equal(await rewardControl.methods.harnessGetNumberOfMarkets(true).call(), 0);

            await rewardControl.methods.addMarket(accounts[4], true).send({from: root});
            assert.equal((await rewardControl.methods.allMarkets(true,0).call()).toString(), accounts[4].toString());
            assert.equal(await rewardControl.methods.harnessGetNumberOfMarkets(true).call(), 1);

            await rewardControl.methods.addMarket(accounts[5], true).send({from: root});
            assert.equal((await rewardControl.methods.allMarkets(true,0).call()).toString(), accounts[4].toString());
            assert.equal((await rewardControl.methods.allMarkets(true,1).call()).toString(), accounts[5].toString());
            assert.equal(await rewardControl.methods.harnessGetNumberOfMarkets(true).call(), 2);

            await assert.revert(rewardControl.methods.addMarket(accounts[6], true).send({from: accounts[1]}), "revert non-owner");
            assert.equal((await rewardControl.methods.allMarkets(true,0).call()).toString(), accounts[4].toString());
            assert.equal((await rewardControl.methods.allMarkets(true,1).call()).toString(), accounts[5].toString());
            assert.equal(await rewardControl.methods.harnessGetNumberOfMarkets(true).call(), 2);

            await assert.revert(rewardControl.methods.removeMarket(0, true).send({from: accounts[1]}), "revert non-owner");
            assert.equal((await rewardControl.methods.allMarkets(true,0).call()).toString(), accounts[4].toString());
            assert.equal((await rewardControl.methods.allMarkets(true,1).call()).toString(), accounts[5].toString());
            assert.equal(await rewardControl.methods.harnessGetNumberOfMarkets(true).call(), 2);
        });
        ...
        it("add existing market failed", async () => {
            ...
            await assert.revert(rewardControl.methods.addMarket(accounts[4], true).send({from: root}), "revert Market already exists");
            assert.equal((await rewardControl.methods.allMarkets(true,0).call()).toString(), accounts[4].toString());
            assert.equal((await rewardControl.methods.allMarkets(true,1).call()).toString(), accounts[5].toString());
            assert.equal(await rewardControl.methods.harnessGetNumberOfMarkets(true).call(), 2);

            await assert.revert(rewardControl.methods.addMarket(accounts[5], true).send({from: root}), "revert Market already exists");
            assert.equal((await rewardControl.methods.allMarkets(true,0).call()).toString(), accounts[4].toString());
            assert.equal((await rewardControl.methods.allMarkets(true,1).call()).toString(), accounts[5].toString());
            assert.equal(await rewardControl.methods.harnessGetNumberOfMarkets(true).call(), 2);
        });
    ...
    describe("#{set|get}AlkAddress", async () => {
        it("set/get alk address successfully", async () => {
            const rewardControl = await RewardControl.new().send({from: root});
            await rewardControl.methods.initializer(root, accounts[5], accounts[2], accounts[3]).send({gas: 1000000, from: root});
        ...
        it("set alk address failed with permission", async () => {
            const rewardControl = await RewardControl.new().send({from: root});
            await rewardControl.methods.initializer(root, accounts[6], accounts[2], accounts[3]).send({gas: 1000000, from: root});
            assert.equal(await rewardControl.methods.getAlkAddress().call(), accounts[3]);
    ...
    describe("#setAlkRate", async () => {
        it("set alk rate successfully", async () => {
            const rewardControl = await RewardControl.new().send({from: root});
            await rewardControl.methods.initializer(root, accounts[2], accounts[3], accounts[4]).send({gas: 1000000, from: root});
            assert.equal(await rewardControl.methods.alkRate().call(), 4161910198000000000);
    ...
    describe("#{set|get}AlkemiEarnVerifiedAddress", async () => {
        it("set/get AlkemiEarnVerified address successfully", async () => {
            const rewardControl = await RewardControl.new().send({from: root});
            await rewardControl.methods.initializer(root,  accounts[2], accounts[3], accounts[5]).send({gas: 1000000, from: root});
            ...
            await rewardControl.methods.setAlkemiEarnVerifiedAddress(accounts[4]).send({from: root});
        ...
        it("set the same AlkemiEarnVerified address failed", async () => {
            ...
            await assert.revert(rewardControl.methods.setAlkemiEarnVerifiedAddress(accounts[2]).send({from: root}), "revert The same AlkemiEarnVerified address");
            result = await rewardControl.methods.getAlkemiEarnAddress().call();
            let {0: verifiedAddress2, 1: publicAddress2} = result;
        ...
        it("set empty AlkemiEarnVerified address failed", async () => {
            ...
            await assert.revert(rewardControl.methods.setAlkemiEarnVerifiedAddress("0x0000000000000000000000000000000000000000").send({from: root}), "revert AlkemiEarnVerified address cannot be empty");
            result = await rewardControl.methods.getAlkemiEarnAddress().call();
            let {0: verifiedAddress2, 1: publicAddress2} = result;
            assert.equal(verifiedAddress2, accounts[2]);
        });
