...
contract("RewardControl interface test", function ([root, ...accounts]) {
	describe("#refreshAlkSupplyIndex", async () => {
		it("refreshAlkSupplyIndex when there are multiple markets and multiple suppliers", async () => {
			...
			await rewardControl.methods
				.initializer(root, accounts[2], accounts[2], accounts[3])
			...
			await rewardControl.methods
				.addMarket(marketA, true)
				.send({ gas: 1000000, from: root });
			await rewardControl.methods
				.addMarket(marketB, true)
				.send({ gas: 1000000, from: root });
			await rewardControl.methods
				.addMarket(marketC, true)
				.send({ gas: 1000000, from: root });
			await rewardControl.methods
				.addMarket(marketD, true)
				.send({ gas: 1000000, from: root });
			...
			await rewardControl.methods
				.refreshAlkSupplyIndex(marketA, supplierA, true)
				.send({from: root });
			await mockSupplyBalance(rewardControl, marketA, supplierA, "50");

			// then
			await assertSupplyResults(
				"1.1",
				rewardControl,
				marketA,
				supplierA,
			...
			await rewardControl.methods
				.harnessFastForward(1)
				.send({ gas: 1000000, from: root });
			await rewardControl.methods
				.refreshAlkSupplyIndex(marketA, supplierA, true)
				.send({ gas: 1000000, from: root });
			await rewardControl.methods
				.refreshAlkSupplyIndex(marketA, supplierB, true)
				.send({ gas: 1000000, from: root });
		...
		it("refreshAlkSupplyIndex multiple times should not change anything", async () => {
			...
			// when #1 refresh
			await rewardControl.methods
				.refreshAlkSupplyIndex(marketA, supplier, true)
				.send({ gas: 1000000, from: root });
			// then
			await assertSupplyResults(
				"1",
	...
	describe("#refreshAlkBorrowIndex", async () => {
		it("refreshAlkBorrowIndex when there is one market and one borrower", async () => {
			...
			// when #1 refresh
			await rewardControl.methods
				.refreshAlkBorrowIndex(marketA, borrower, true)
				.send({ gas: 1000000, from: root });
			await mockBorrowBalance(rewardControl, marketA, borrower, "25");

			// then
			await assertBorrowResults(
				"1",
				rewardControl,
				marketA,
				borrower,
		...
		it("refreshAlkBorrowIndex when there are multiple markets and multiple borrowers", async () => {
			...
			await rewardControl.methods
				.refreshAlkBorrowIndex(marketA, borrowerA, true)
				.send({ gas: 1000000, from: root });
			...
			await rewardControl.methods
				...
				.send({ gas: 1000000, from: root });
			await rewardControl.methods
				.refreshAlkBorrowIndex(marketA, borrowerB, true)
				.send({ gas: 1000000, from: root });
	...
	describe("#claimAlk for all markets", async () => {
		it("claimAlk when there is one market and one user borrowing and lending assets in the market", async () => {
			...
			await ALK.methods
				.transfer(rewardControl._address, "70000000000000000000000000")
				.send({ from: root });
			await rewardControl.methods
				.initializer(root, accounts[2], accounts[2], ALK._address)
				.send({ gas: 1000000, from: root });
			let marketA = accounts[1];
			let supplier = accounts[4];
			...
			await assertBorrowResults(
				...
				"8323820400000000000"
			); // 8.323820400000000000 ALK (+ 1.040477550000000000 ALK)
			...
			let rewardControlAlkBalance = await ALK.methods
				...
				.call();
			assert.equal(
				rewardControlAlkBalance.toString(),
				"69999983352359200000000000"
			); // 69999983.352359200000000000 ALK
			await assertSupplyResults(
				"6",
				rewardControl,
				marketA,
		...
		it("claimAlk when there are multiple markets and multiple users (ALK accrued from multiple markets)", async () => {
			...
			await rewardControl.methods
				.refreshAlkSupplyIndex(marketB, supplierA, true)
				.send({ gas: 1000000, from: root });
			...
			await assertSupplyResults(
				...
				"1007371446136363636"
			); // 1.007371446136363636 ALK (it stays the same due to the first time supplierA use marketB)
		...
		it("claimAlk multiple times should not change anything", async () => {
			...
			let participantAlkBalance_2 = await ALK.methods
				.balanceOf(supplier)
				.call();
			assert.equal(participantAlkBalance_2.toString(), "1040477550000000000");
			let rewardControlAlkBalance_2 = await ALK.methods
				.balanceOf(rewardControl._address)
				.call();
			assert.equal(
				rewardControlAlkBalance_2.toString(),
				"69999998959522450000000000"
			);
			await assertSupplyResults(
				"5",
			...
			let participantAlkBalance_3 = await ALK.methods
				.balanceOf(supplier)
				.call();
			assert.equal(participantAlkBalance_3.toString(), "1040477550000000000");
			let rewardControlAlkBalance_3 = await ALK.methods
				.balanceOf(rewardControl._address)
				.call();
			assert.equal(
				rewardControlAlkBalance_3.toString(),
				"69999998959522450000000000"
			);
			await assertSupplyResults(
				"6",
	...
	describe("#claimAlk for a single market", async () => {
		it("claimAlk when there is one market and one user borrowing and lending assets in the market", async () => {
			// given
			const rewardControl = await RewardControl.new().send({ from: root });
			const ALK = await EIP20.new(
		...
		it("claimAlk when there are multiple markets and multiple users (ALK accrued from one market)", async () => {
			...
			await rewardControl.methods
				...
				.send({ gas: 1000000, from: root });
			await rewardControl.methods
				.claimAlk(supplierA, marketA)
				.send({ gas: 1000000, from: root });
			await rewardControl.methods
				.claimAlk(supplierA, marketB)
				.send({ gas: 1000000, from: root });
			await rewardControl.methods
				.claimAlk(supplierA, marketC)
				.send({ gas: 1000000, from: root });
			await rewardControl.methods
				.claimAlk(supplierA, marketD)
				.send({ gas: 1000000, from: root });
		...
		it("claimAlk when there are multiple markets and multiple users (ALK accrued from multiple markets)", async () => {
			...
			// when #5.1.1 claim ALK for supplierA (marketA) on 1 block away
			await rewardControl.methods
				.harnessFastForward(1)
				.send({ gas: 1000000, from: root });
			await rewardControl.methods
			...
			// then (supplierA), marketA
			let supplierAAlkBalance_1 = await ALK.methods.balanceOf(supplierA).call();
			// (marketA) 1.461398013409090908 ALK = 1.007371446136363636 ALK (from last ALK accrued) + 0.454026567272727272 ALK (1135066418181818180 * 40% * 1 block)
			// (marketA + marketB) 1.713634995227272725 ALK = 1.461398013409090908 ALK + 0.252236981818181817 ALK (1008947927272727271 * 25% * 1 block)
			assert.equal(supplierAAlkBalance_1.toString(), "1461398013409090908");
			let rewardControlAlkBalance_1 = await ALK.methods
				.balanceOf(rewardControl._address)
				.call();
			assert.equal(
				rewardControlAlkBalance_1.toString(),
				"69999998538601986590909092"
			);
			await assertSupplyResults(
				"5.1.1",
			...
			// when #5.1.2 claim ALK for supplierA (marketB) on 1 block away
			await rewardControl.methods
				.claimAlk(supplierA, marketB)
				.send({ gas: 1000000, from: root });

			// then (supplierA), marketA + marketB
			let supplierAAlkBalance_2 = await ALK.methods.balanceOf(supplierA).call();
			// (marketA) 1.461398013409090908 ALK = 1.007371446136363636 ALK (from last ALK accrued) + 0.454026567272727272 ALK (1135066418181818180 * 40% * 1 block)
			// (marketA + marketB) 1.713634995227272725 ALK = 1.461398013409090908 ALK + 0.252236981818181817 ALK (1008947927272727271 * 25% * 1 block)
			assert.equal(supplierAAlkBalance_2.toString(), "1713634995227272725");
			let rewardControlAlkBalance_2 = await ALK.methods
				.balanceOf(rewardControl._address)
				.call();
			assert.equal(
		...
		it("claimAlk when market is not included in the reward program", async () => {
			// given
			const rewardControl = await RewardControl.new().send({ from: root });
			const ALK = await EIP20.new(
				"70000000000000000000000000",
			...
			await rewardControl.methods
				.initializer(root, accounts[2], accounts[2], ALK._address)
				.send({ gas: 1000000, from: root });
			let unregisteredMarket = accounts[4];
			let supplier = accounts[5];

			// when and then
			await assert.revert(
				rewardControl.methods
					.claimAlk(supplier, unregisteredMarket, true)
					.send({ gas: 1000000, from: root }),
	...
	async function mockMarketLiquidity(
		rewardControl,
		market,
		totalSupply,
		totalBorrows
		...
		await rewardControl.methods
			.harnessSetMarketTotalSupply(market, totalSupply)
			.send({
				gas: 1000000,
				from: root,
			});
		await rewardControl.methods
			.harnessSetMarketTotalBorrows(market, totalBorrows)
			.send({
				gas: 1000000,
				from: root,
			});
	...
	async function mockSupplyBalance(
		rewardControl,
		market,
		supplier,
		supplyBalance
		...
		await rewardControl.methods
			.harnessSetSupplyBalance(market, supplier, supplyBalance)
			.send({
				gas: 1000000,
				from: root,
			});
	...
	async function mockBorrowBalance(
		rewardControl,
		market,
		borrower,
		borrowBalance
		...
		await rewardControl.methods
			.harnessSetBorrowBalance(market, borrower, borrowBalance)
			.send({
				gas: 1000000,
				from: root,
			});
	...
	async function assertSupplyResults(
		label,
		rewardControl,
		market,
		supplier,
		expectedAlkSpeed,
		expectedSupplierIndex,
		expectedAlkAccrued
	) {
		let speed = await rewardControl.methods.alkSpeeds(true, market).call();
		let alkSupplierIndex = await rewardControl.methods
			.alkSupplierIndex(true, market, supplier)
			.call();
