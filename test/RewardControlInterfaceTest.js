...
contract('RewardControl interface test', function ([root, ...accounts]) {
    ...
    describe("#claimAlk for a single market", async () => {
        it("claimAlk when there is one market and one user borrowing and lending assets in the market", async () => {
            ...
            // when claim on the same block
            await rewardControl.methods.claimAlk(supplier, marketA).send({gas: 1000000, from: root});
        ...
        it("claimAlk when there are multiple markets and multiple users (ALK accrued from one market)", async () => {
            ...
            // when #5 claim ALK for supplierA on 1 block away
            await rewardControl.methods.harnessFastForward(1).send({gas: 1000000, from: root});
            await rewardControl.methods.claimAlk(supplierA, marketA).send({gas: 1000000, from: root});
            await rewardControl.methods.claimAlk(supplierA, marketB).send({gas: 1000000, from: root});
            await rewardControl.methods.claimAlk(supplierA, marketC).send({gas: 1000000, from: root});
            await rewardControl.methods.claimAlk(supplierA, marketD).send({gas: 1000000, from: root});
        ...
        it("claimAlk when there are multiple markets and multiple users (ALK accrued from multiple markets)", async () => {
            ...
            let rewardControlAlkBalance_1 = await ALK.methods.balanceOf(rewardControl._address).call();
            assert.equal(rewardControlAlkBalance_1.toString(), "69999998538601986590909092");
            await assertSupplyResults("5.1.1", rewardControl, marketA, supplierA,
                "1135066418181818180",
                "49375389190909090880000000000000000000000000000000000",
                "0"); // 0 ALK
            ...
            let rewardControlAlkBalance_2 = await ALK.methods.balanceOf(rewardControl._address).call();
            assert.equal(rewardControlAlkBalance_2.toString(), "69999998286365004772727275");
            await assertSupplyResults("5.1.2", rewardControl, marketA, supplierA,
                "1135066418181818180",
                "49375389190909090880000000000000000000000000000000000",
                "0"); // 0 ALK
        ...
        it("claimAlk when market is not included in the reward program", async () => {
            ...
            // when and then
            await assert.revert(rewardControl.methods.claimAlk(supplier, unregisteredMarket).send({gas: 1000000, from: root}), "revert Market does not exist");
        });
    });
