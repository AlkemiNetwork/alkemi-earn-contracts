...
const { assets, checksum, getExpMantissa, range, assert } = require("./Utils");
...
contract("ErrorReporterHarness", function([root, ...accounts]) {
	it("emits a failure", async () => {
		const errorReporterHarness = await ErrorReporterHarness.new().send({
			from: root,
		});
		const result = await errorReporterHarness.methods
			.harnessPleaseFail()
			.send({ from: root });
	...
	it("emits an opaque failure", async () => {
		...
		const result = await errorReporterHarness.methods
			.harnessPleaseFailOpaque(opaqueFailure)
			.send({ from: root });
