...
contract("AlkemiEarnVerifiedHarness", function([root, ...accounts]) {
	describe("_supportMarket", async () => {
		...
		it("fails if asset has no price", async () => {
			const alkemiEarnVerified = await AlkemiEarnVerifiederifiedHarness.new().send(
				{
					from: root,
				}
			);
		...
		it("succeeds and sets market", async () => {
			...
			assert.equal(
				await alkemiEarnVerified.methods.collateralMarkets(0).call(),
				asset,
		...
		it("succeeds works a second time", async () => {
			...
			assert.withinPercentage(
				1e18,
				Number(
					await alkemiEarnVerified.methods.harnessGetSupplyIndex(asset).call()
				),
				1e-10,
			...
			assert.withinPercentage(
				1e18,
				Number(
					await alkemiEarnVerified.methods.harnessGetBorrowIndex(asset).call()
				),
				1e-10,
		...
		it("adds two assets", async () => {
			...
			assert.equal(
				await alkemiEarnVerified.methods.collateralMarkets(1).call(),
			...
			assert.withinPercentage(
				...
				Number(
					await alkemiEarnVerified.methods
						.harnessGetSupplyIndex(omg._address)
						.call()
				),
				1e-10,
			...
			assert.withinPercentage(
				...
				Number(
					await alkemiEarnVerified.methods
						.harnessGetSupplyIndex(drgn._address)
						.call()
				),
				1e-10,
	...
	describe("_setRiskParameters", async () => {
		...
		// 	const alkemiEarnVerified = await AlkemiEarnVerifiedHarness.new().send({ from: root });
	...
	describe("harnessCalculateInterestIndex", async () => {
		it("calculates correct value for exact value", async () => {
			const alkemiEarnVerifiedHarness = await AlkemiEarnVerifiedHarness.new().send(
				{
					from: root,
				}
			);
	...
	describe("withdraw", async () => {
		it("returns error and logs info if contract is paused", async () => {
			...
			// Give protocol cash
			await OMG.methods
				.harnessSetBalance(alkemiEarnVerifiedHarness._address, 500)
				.send({ from: root });
		...
		it("returns error and logs info if protocol has insufficient cash for withdrawal", async () => {
			...
			await OMG.methods
				.harnessSetBalance(alkemiEarnVerifiedHarness._address, 1)
				.send({ from: root });
	...
	describe("getSupplyBalance", async () => {
		it("returns balance in happy path when there is no interest to accumulate", async () => {
			...
			const customerBalance = await alkemiEarnVerifiedHarness.methods
